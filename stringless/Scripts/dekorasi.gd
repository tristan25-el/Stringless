...
# Ketinggian Y persis seperti di tiles.gd
...
func _process(_delta: float) -> void:
	...
	if main_node and "current_game_time" in main_node:
		...
		# Hapus dekorasi jika sudah lewat jauh dari layar (progress 1.5)
		if progress >= 1.5: 
			queue_free()
...
func update_perspective() -> void:
	...
	# Rumus 3D yang persis sama dengan rintangan
	...
	# Perbaiki letak poros gambar agar rak/manekin tidak melayang
	if texture:
		centered = true
